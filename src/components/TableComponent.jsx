...
const TableComponent = () => {
    ...
    return (
        <>
            <div className='flex flex-col mt-9 border border-gray-100 rounded'>
                {
                    CryptoData ?
                        <table className='w-full table-auto'>
                            <thead className=' capitalize text-base text-gray-100 font-medium
                            ...
                            <tbody>
                                {
                                    CryptoData.map((data) => {
                                        return (
                                            <tr key={data.id} className='text-center text-base border-b border
                            border-gray-100 hover:bg-gray-200 last:border-b-0'>
                                                <td className='py-4 flex items-center uppercase '>
                                                    <SavedBtn data={data} />
                                                    <img src={data.image} className='w-[1.2rem] h-[1.2rem] mx-1.5'
                                                        alt={data.name} />
                                                    <span>
                                                        <Link to={`/${data.id}`}>
                                                            {data.symbol}
                                                ...
                                                <td className={`py-4 ${data.price_change_percentage_1h_in_currency > 0
                                                    ? "text-green" : 'text-red'}`}>{Number(data.price_change_percentage_1h_in_currency).toFixed(2)}</td>
                                                <td className={`py-4 ${data.price_change_percentage_24h_in_currency > 0
                                                    ? "text-green" : 'text-red'}`}>{Number(data.price_change_percentage_24h_in_currency).toFixed(2)}</td>
                                                <td className={`py-4 ${data.price_change_percentage_7d_in_currency > 0
                                                    ? "text-green" : 'text-red'}`}>{Number(data.price_change_percentage_7d_in_currency).toFixed(2)}</td>
                                            </tr>
                                        )
                                    })
                                }
            ...
            <div className='flex items-center mt-4 capitalize h-[2rem] justify-between'>
                <span>Data provided by <a className='text-cyan' href="http://www.coingecko.com"
                    target='_blank'>Coingecko</a></span>
                <Pagination />
            </div>
        </>
    )
